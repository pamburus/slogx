...
func New(theme *Theme, cfg *Config) *StyleCache {
	c := &StyleCache{
		...
		Logger:              st(theme.Logger).append(st(theme.LoggerMessageSep).render(cfg.LoggerMessageSep)).ws(),
	...
	for i := 0; i < NumLevels; i++ {
		...
		c.LevelExpansion[i] = st(theme.Level[i]).ws().render(cfg.LevelExpansions[i])
...
type StyleCache struct {
	...
	LevelExpansion      [NumLevels]string
	Logger              Style
	Message             StringStyle
	Key                 Style
	ExpandedKey         Style
